...
if __name__ == "__main__":
    """Depending on your needs, you can change the number of tweets to embedd per minute N_tweets"""
    ...
    if mode == "train":
        path_data = "challenge_data/train/train_tweets"
        path_write = "challenge_data/train/train_BERT"
    
    else:
        path_data = "challenge_data/eval/eval_tweets"
        path_write = "challenge_data/eval/eval_BERT"
    ...
    for i in range(len(data_files)):
        ...
        ### Subsampling
        sampled_df = df.groupby("ID", group_keys=False).apply(
            lambda x: x.sample(
                n=min(len(x), N_tweets),
                replace=len(x) < N_tweets
