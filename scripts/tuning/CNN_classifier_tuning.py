...
from src.CNN.CNNBinaryClassifier import CNNBinaryClassifier
import pandas as pd
import torch
from src import utils
from src.CNN import DL_utils
from torch.utils.data import DataLoader, WeightedRandomSampler
from src.TweetDataset import TweetDataset, NextTweetDataset
from src.CNN.DL_utils import compute_accuracy
import pickle
...
if __name__ == "__main__":
    ...
    columns_to_drop = ["Timestamp", "Tweet"]
    columns_to_drop = [col for col in columns_to_drop if col in df_BERT.columns]
    df_train, df_test = utils.MATCH_ID_train_test_split(df_BERT.drop(columns=columns_to_drop))  
    ...
    model = CNNBinaryClassifier(**model_params).to(device)
   ...
   # %% Save model as pickle
    save_path_pickle = "models/CNN_classifier_128.pkl"
    with open(save_path_pickle, "wb") as f:
        pickle.dump(model, f)
    print(f"Model saved as pickle at {save_path_pickle}")
